...
Follow-up 2026-09-12 (field repro: 429 reported as clean completion): `hydrateLatestText` replaced by `hydrateLatestOutcome` — one read supplies both latest text and `messageErrorDetail` (the `AssistantMessage.error → ApiError.data.message` signal, which event status cannot see). A terminal `idle` over an errored stream now notifies `error` with the provider detail instead of `completed successfully` + `(completed)`. Same never-throws contract.
