...
Smoke-tested live 2026-09-25, two levels deep, and the wording is the part that needed fixing. The host places the notification in the parent's context **as a user message while the call it was dialed on is still open** — so "the parent session has not answered" was false the moment the parent had read it, and the parent is exactly who reads that line. The honest claim is `dialed, unacknowledged`. The correction itself was confirmed from the other side: a grandchild whose parent's turn ended reads `delivered in 1 attempt(s)`, while the record of a write dialed at a parent still mid-turn stays PENDING — which is the truth, and not a stuck state.
