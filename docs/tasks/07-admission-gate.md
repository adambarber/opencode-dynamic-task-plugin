...
Follow-up 2026-09-22 (allow any agent by default): `DEFAULT_CONFIG.blockedAgents` is now `[]` — lineage, depth, and recursion policy bound the real hazards, not the agent name. The blocklist mechanism (config/tuple/env/file, `validateAgent` funnel, contract tests) is unchanged operator policy.
...
Follow-up 2026-09-22 (revival skips re-validation): `reviveRetainedTask` re-checks `maxConcurrent` but never lineage or `maxDepth` — policy binds at spawn, so lowering `maxDepth` never strands an already-running lineage. Deliberate, now stated.
