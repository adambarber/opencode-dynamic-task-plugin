...
**Follow-up 2026-09-25 (the response envelope is a transport fact, so this module owns it):** the SDK client resolves every call to `{ data, request, response }` — its `responseStyle` default is "fields", per `@default 'fields'` in `types.gen.d.ts`, not the bare payload. A reader that looks for a field at the top level of the resolved value is reading a level too high, and that failure is silent: it finds nothing rather than throwing. Three readers had each grown a private ladder for it (`extractMessages` checked `data` then `body.messages`, `validateSessionResult` walked five keys recursively, `parseAgentList` checked `agents` then `data` then `Object.values`) — three tolerances for one transport fact, and the status read added in Cycle 5 had none, so it answered "no session" for every live child in production while the suite stayed green because the mocks returned bare payloads. `hostPayload` is now the single unwrap (the SDK's `data` plus the route envelopes those three ladders had grown), all four readers route through it, and the harness resolves calls into the same envelope so the mock cannot hide the class again. The invariant scans the whole tree and asserts exactly one ladder and that it lives here — a whitelist would have filtered a reader's own ladder out of the corpus and reported green.
