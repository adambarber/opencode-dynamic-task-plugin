...
- **Settle-while-steering:** the claim is re-validated after the awaited abort — a task settled mid-steer (event or interrupt) receives no replacement prompt and gets a truthful settled-as-X report instead of a false "Steer sent". Firing there would launch an untracked turn on a settled session.
...
- **Residual risk (accepted):** if the server never emits the aborted turn's echo, the claim eats the replacement turn's genuine terminal event and the task strands active (recoverable by `task_interrupt`). This inverts the pre-steer failure (wrong settlement + lost work) into a visible stall, and matches the interrupt path's own documented assumption that aborts provoke idle/error events.
