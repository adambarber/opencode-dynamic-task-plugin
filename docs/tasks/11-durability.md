...
- **Ledger v3, one migration.** `{ version: 3, tasks: {...retained}, active: {...}, pending: [...] }`. The v2 reader keeps discarding non-v3 wholesale (same policy as v1→v2: a partial past is worse than a clean one). `active` entries carry the full `ActiveTaskState` minus advisory fields (`lastNotice` never persists — a fresh boot owns no in-flight claims). `pending` entries are `{ childSessionId, kind, text, attempts }` written *before* the first delivery attempt and deleted on success — the only new write site, beside the retained observer.
- **Boot reconciliation is a one-shot pass, not a timer.** For each restored active: one live liveness read (the same one the read tools make — `session.status` for the server's turn status, the child's transcript for its own last message; `session.get` is not a source, its `Session` type carries no status field). Turn running → re-armed as active (its natural terminal event settles it; the generation it belonged to died with the process, so settle-dedup starts empty — safe because no delivery could have happened for it). No turn running → settle `error` ("session vanished across restart") and queue its notice in `pending`. No polling, no clocks — Task 09 stands.
